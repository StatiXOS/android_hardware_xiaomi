...
::ndk::ScopedAStatus ConsumerIr::getCarrierFreqs(std::vector<ConsumerIrFreqRange>* _aidl_return) {
    ...
    for (int32_t i = 0; i < rangeVec.size(); i++) {
        (*_aidl_return)[i].minHz = static_cast<uint32_t>(rangeVec[i].minHz);
        (*_aidl_return)[i].maxHz = static_cast<uint32_t>(rangeVec[i].maxHz);
    }
...
::ndk::ScopedAStatus ConsumerIr::transmit(int32_t carrierFreqHz,
    ...
    rc = ioctl(lircFd, LIRC_SET_SEND_CARRIER, &carrierFreqHz);
    if (rc < 0) {
